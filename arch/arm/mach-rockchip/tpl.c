...
int i2c1_send_start_bit(void)
{
	...
	writel(I2C_CON_EN | I2C_CON_START, &iregs->con);
	writel(I2C_STARTIEN, &iregs->ien);

	start = get_timer(0);
	while (1) {
		if (readl(&iregs->ipd) & I2C_STARTIPD) {
			writel(I2C_STARTIPD, &iregs->ipd);
			break;
		}
...
void rk805_probe(void)
{
	...
	rk805_read(id_msb, &msb, 1);

	id_lsb = 24;
	lsb = 0;

	rk805_read(id_lsb, &lsb, 1);
...
/* setting mux of GPIO2-4 to 2, setting pull of GPIO2-4 to 1 */
        writel(0x3000200, GRF_GPIO2A_IOMUX);
        writel(0x3000000, GRF_GPIO2A_P);
/* setting mux of GPIO2-5 to 2, setting pull of GPIO2-5 to 1 */
        writel(0xc000800, GRF_GPIO2A_IOMUX);
        writel(0xc000000, GRF_GPIO2A_P);

/* setting mux of GPIO1-24 to 0, setting pull of GPIO1-24 to 5 */
        writel(0x30000, GRF_GPIO1D_IOMUX);
        writel(0x30001, GRF_GPIO1D_P);
}
...
void buck1_set_suspend_value(int buck, int uvolt)
{
	...
	buck1_clrsetbits(0x30, mask, val);
}
...
void board_init_f(ulong dummy)
{
	...
	buck1_set_suspend_enable(0, 1);
	buck1_set_suspend_value(0, 1000000);
	buck1_set_enable(0, 1);
